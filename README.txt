...
- in_game/  游戏内数据根（与游戏本体 in_game/ 同构，同名文件覆盖原版）
  ...
  - common/disasters/  ★两个灾难：夺舍(blades_and_thrones_disaster.txt)/政变(blades_and_thrones_military_coup.txt)；军政府已并入政变链（2024-08 重构，不再独立成灾难）
...
- blades_and_thrones_loyalty_events.txt 忠诚/哗变/政变/军政府抉择路线（loyalty.1-4）
- （junta_events.txt 已随 2024-08 军政府重构删除——军政府不再有稳定度/派系子系统）
...
- 计数变量的初始化不能只依赖"创建路径"：旧档/调试路径/已删选项残留的军团
  宗主侧可能没有计数变量（实测 987 条：索饷拒绝选项 change_variable 与
  var: 比较双双报错）。防御写法 = 事件选项 hidden_effect 内先
  `if NOT has_variable → set_variable value = 0` 再 change_variable
  （幂等，不重置已有计数）；同批 loc 错误（688 条 "Failed converting
  statement"）是 GetValue 缺 |V0 的旧文件，与脚本错误独立。
...
- event blades_and_thrones_loyalty.3  政变处理事件（含军人夺权→军政府抉择链）
...
- 链主线：佣兵依赖→军事危机→夺舍灾变→结局后军人自恃（12月）→忠诚请愿
  ...
  军政府（执政 5 年）→核心抉择（还政/制度化/铁腕）→复辟余波（12月）→再哗变
  风险。螺旋而非堆叠。
...
- 阈值放宽的接续变量（忠诚<0.4 即可哗变）：blades_and_thrones_soldiers_emboldened
  ...
  （军政府还政后军人被夺权不甘 12月；抉择一生一次，永久修正不可重复领取）。
...
- 3 个专属人格：
  * ai_trusted_company（王室军团）：忠顺纪律——宗主选择"赐予忠诚待遇"/"转为王室军团"
    感化军团时获得
...
- AI 决策按性格加权：claim_foothold（夺取立足地）ai_will_do 中军阀 +80 /
  佣兵 +20 / 王室军团 -40；company.1 性质抉择按军团性格决定宗主信任度。
...
- 每隔数月，佣兵军团国会索要巨额军饷（按形态分四档：雇佣津贴 240 / 军费拨款
  300 / 老兵雇佣津贴 420 / 大军团军费拨款 600 金，见年度账单 blades_and_thrones_company.9）：
...
- 五个转化：
  ...
  * 组建军团：选择一支常备军，组建为无地军团国（你的附属国，名为“佣兵自由团”），随后三选一：“赐予忠诚待遇”（高忠诚、正常进贡、不索饷不闹独立的忠诚远征军团）、“亲自统领军团”（玩家切换扮演）、“转为王室军团”（听令、防御必参、军费拨款供养；2024-08 起不再有“放任自立”——不忠附庸只由夺舍和解自立路径产生，索饷/独立链保留）。
...
- AI 会主动使用五个转化：
  ...
  * 组建军团：军事危机≥70 且和平时外派部队（危机减压）；军团性质选择事件 AI 按性格加权（王室军团人格 ×2、佣兵人格 ×0.5，转王室军团选项仅 ×0.15），“亲自统领军团”选项已加玩家门槛防止 AI 误切换。
...
- 军事政变对 AI 同样开放（无玩家门槛，2024-08 全面放开）。
...
- “四海为家”政府改革（无主军团专属：country_type = army 且非附庸，流亡者/自由军团/佣兵军团可选，原“购粮权”）：把无地国流浪通行机制并入改革本身——拥有该改革的国家，每月自动与外交范围内所有非交战国家建立通行关系（mod 自定义隐藏关系 blades_and_thrones_landless_passage，含 military access + food access，5 年续期），同时军队驻留的每个有主地点自动向地主建立购粮关系（food access，10 年续期）。走到哪都有粮、有路。
...
- 军事政变中“接受军人接管”会让国家获得“军政府”政府改革，而不是只施加临时修正。该改革以政变变量门控：正常游玩不出现在改革面板（不能直接选取），只能通过军事政变获得；改革 locked = always yes（玩家无法移除/替换，只能走完军政府流程）；执政期 5 年（1825 天），期间军政府修正与 AI 好战倾向持续生效，期满由 loyalty.3 的接续事件自动弹出军政府核心抉择（loyalty.4），抉择后移除改革。
- 政变处理（loyalty.3）：镇压 / 政治妥协 / 接受军人接管三选一，AI 按军队强势度、士兵满意度和王室正统加权（士兵不满或军队超规模时倾向接受接管）；需已接纳“职业军队”思潮（professional_armies，1337 早期不会发生）。
- 军政府核心抉择（loyalty.4，2024-08 重构替代原稳定度博弈灾难）：还政王室（永久“文官复位”+正统大额恢复，士兵不满）/ 制度化巩固（永久“军政府制度化”+声望，士兵满意）/ 铁腕清洗（贵族教士市民人口各 -10% + 永久“清洗后的铁腕秩序”+10 年“军政府清洗余波”）。三选一了结，整条政变链共 2 个弹窗（loyalty.3 → 5 年后 loyalty.4）；军政府一生一次（coup_resolved 防重复政变）。
- （原“军政府支持/派系/稳定度”子系统已随 2024-08 重构全部删除——不再有掌控力数值、支持事件与派系斗争。）
- 调试政变链：event blades_and_thrones_loyalty.3（处理政变）→ 选“接受军人接管”→ 5 年（1825 天）后自动弹 loyalty.4 抉择（也可直接用 event blades_and_thrones_loyalty.4 跳过等待）
- 战争反馈：进攻与防守分开结算。防守胜利获得更高正统性奖励；进攻胜利声望更高。进攻战败留下的“战败耻辱”对军队忠诚影响更大。长期战争中进攻方从18个月开始、防守方从36个月开始承受士兵满意度下降。
- 调试战争反馈：event blades_and_thrones_war.99
- 军事情报汇总：每6个月为人类玩家弹出一份报告，显示本月军饷、军队忠诚、士兵满意度、佣兵威胁、战争持续月数与军事危机；军事转化面板顶部同样显示这份报告。
...
平衡性数据表（中英双语，含军饷/忠诚/危机/夺舍/特权/时代联动/军政府/军团补贴与索饷/出租/团练全部数值）：
